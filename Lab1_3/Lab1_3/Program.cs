...
class Person
{
    ...
    public static int MaxCarCount { get; } = 3;
    ...
    public Person(string firstName, string lastName, string address) : this()
    {
    ...
    {
        if (_carCount >= MaxCarCount)
        {
            Console.WriteLine($"{_firstName} {_lastName} nie może posiadać więcej niż {MaxCarCount} samochody.");
            return;
        }
        _registrationNumbers[_carCount++] = registrationNumber;
    }
    ...
    public void RemoveCarRegistrationNumber(string registrationNumber)
    {
        ...
        Console.WriteLine($"{_firstName} {_lastName} nie posiada samochodu z numerem rejestracyjnym {registrationNumber}.");
    }

    public override string ToString()
    {
        string info = $"Person | Name: {_firstName} {_lastName}, Address: {_address}, Cars: ";
        foreach (var reg in _registrationNumbers)
        {
            if (reg != null)
                info += reg + " ";
        }
