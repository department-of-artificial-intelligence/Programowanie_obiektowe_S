...
class Program
{
    static void Main(string[] args)
    {
		...
		Item? foundedItemById = catalog.FindItemBy(item => item.Id == 1);
		Item? foundedItemByTitle = catalog.FindItemBy(item => item.Title == searchedValue);
		Item? foundedItemByDateRange = catalog.FindItemBy(item => item.DateOfIssue >= new DateTime(2014, 12, 31) &&
		...
		var foundedByLambda = library.FindItemBy(x => x.Publisher == "Springer");
