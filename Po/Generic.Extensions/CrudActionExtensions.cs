...
namespace Generic.Extensions
{
    public static class CrudActionExtensions
    {
        public static IList<TObjectType>? Set<TObjectType>(this IContainer containerObject)
        {
            ...
            {
                // Możesz tu również rzucić wyjątek lub zwrócić pustą listę, w zależności od wymagań aplikacji.
        ...
        public static IList<TObjectType> GetList<TObjectType>(this IContainer container, Func<TObjectType, bool>? searchPredicate = null)
        {
            var containerObjectType = container.GetType();
            var propertyInfo = containerObjectType.GetProperties().FirstOrDefault(
                p=>p.PropertyType == typeof(IList<TObjectType>));
        ...
        public static IContainer Add<TObjectType>(this IContainer container, TObjectType obj)
        {
            ...
            if (propertyInfo == null)
            {
                // Jeśli właściwość nie istnieje, utwórz nową listę i przypisz ją do właściwości kontenera
                var newList = new List<TObjectType> { obj };
                propertyInfo?.SetValue(container, newList);
            }
            else
            {
                if (list == null)
