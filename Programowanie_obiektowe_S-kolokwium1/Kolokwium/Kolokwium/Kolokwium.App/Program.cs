...
class Kolos
{
    public static void Main()
    {
        ...
        Subject sub1 = school.CreateSubject("Programowanie obiektowe", "Przedmiot opisujący programowanie obiektowe w jezyku C#");
        Subject sub2 = school.CreateSubject("Bazy danych", "Przedmiot opisujący bazy danych oraz jezyk SQL");
        ClassRoom classRoom1 = school.CreateClassRoom(1, 30, true);

        for (int i = 1; i <= 30; i++) 
            classRoom1.AddSeat(i);

        ClassRoom classRoom2 = school.CreateClassRoom(1, 60, true);

        for (int i = 1; i <= 60; i++) 
            classRoom2.AddSeat(i);

        Teacher teacher1 = school.CreateTeacher("Jan", "Kowalski");
        Teacher teacher2 = school.CreateTeacher("Piotr", "Nowak");
        Lesson lesson1 = school.CreateLesson(classRoom1, sub1, "Wtorek", new TimeSpan(12, 0, 0), new TimeSpan(14, 0, 0), teacher1);

        Lesson lesson2 = school.CreateLesson(classRoom2, sub2, "Czwartek", new TimeSpan(12, 0, 0), new TimeSpan(14, 0, 0), teacher2);
        ...
        /*
        var teachers1 = new List<Teacher>();
        teachers1.AddRange<Teacher>(new List<Teacher>() { new Teacher("T1", "LN2", new List<Lesson>()), new Teacher("T2", "LN3", new List<Lesson>()) });
        teachers1.ForEach(tea => tea.Name = "asdfasdf" );
        teachers1.ForEach(tea => Console.WriteLine(tea)); //dziala :)
        */ 
        Console.ReadKey();
