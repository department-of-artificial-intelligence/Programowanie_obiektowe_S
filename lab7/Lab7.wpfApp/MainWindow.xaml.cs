...
namespace Lab7.wpfApp
{
    public partial class MainWindow : Window
    {
        ...
        private void OnButtonOperationClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                // Spróbuj pobrać drugą liczbę z display.Text
                string[] parts = display.Text.Split(new char[] { '+', '-', '*', '/' }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length >= 2 && double.TryParse(parts.Last().Trim(), NumberStyles.Any, culture, out double tempSecond))
                {
                    firstNumber = currentOperation(firstNumber, tempSecond);
                    display.Text = firstNumber.ToString(culture);
                }
            }
            else if (double.TryParse(display.Text, NumberStyles.Any, culture, out double parsed))
                firstNumber = parsed;
        ...
        private void OnButtonResultClick(object sender, RoutedEventArgs e)
        {
            ...
            if (double.TryParse(parts.Last().Trim(), NumberStyles.Any, culture, out secondNumber))
            {
                try
                {
